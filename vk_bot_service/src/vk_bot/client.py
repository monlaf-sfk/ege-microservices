...
class APIClient:
    ...
    async def register_user(self, vk_id: int, first_name: str, last_name: str | None) -> UserResponse:
        payload = UserCreate(vk_id=vk_id, first_name=first_name, last_name=last_name)
    ...
    async def add_score(self, vk_id: int, subject: str, score: int) -> ScoreResponse:
        payload = ScoreCreate(user_vk_id=vk_id, subject=subject, score=score)
        data = await self._post("/scores/", payload.model_dump(mode='json'))
        return ScoreResponse(**data)

    async def get_my_scores(self, vk_id: int) -> List[ScoreResponse]:
        data = await self._get("/scores/", params={"vk_id": vk_id})
